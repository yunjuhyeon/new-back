...
✔ 타임리프 템플릿 반환 과정에서 패치 문제를 해결하려니 경로 설정과 데이터 매핑에서 막막한 점이 많았다. 처음엔 템플릿 파일이 없어서 발생한 문제인지, 데이터 전달 과정에서 오류가 있는지조차 명확히 알 수 없었고, 검색을 통해 해결하려 해도 직접 코드와 설정을 확인하며 하나씩 점검하는 수밖에 없었다.
...
❗문제 상황❗
<br>
데이터를 패치한 후 페이지네이션을 구현하려 했으나, 데이터를 동적으로 불러오는 과정에서 페이지 이동 시 원하는 데이터가 정확히 반영되지 않는 문제가 발생
<br>
<br>
🚨문제 원인🚨<br>
패치 방식으로 데이터를 가져올 때, 기존 페이지 상태를 유지하지 못하거나 잘못된 페이지 인덱스를 참조하여 데이터 매핑이 어긋남
<br>
<br>
🚀해결 방법🚀<br>
패치 요청 시 페이지 인덱스와 페이지 크기를 정확히 전달하도록 API 호출 로직을 수정하고, 프론트엔드와 백엔드 간 데이터를 동기화하여 페이지 상태를 유지하도록 구현. 
<br> 
